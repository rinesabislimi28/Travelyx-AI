...
// Create Groq client using API key from environment variables
const groq = new Groq({
  apiKey: process.env.GROQ_API_KEY
});

/**
 * Generates a structured travel plan using Travelyx-AI
 * @param {string} userPrompt - The user's travel request
 * @returns {Promise<Object|null>} Parsed JSON travel plan
 */
...
IMPORTANT:
Return ONLY pure JSON. Do NOT wrap the JSON with markdown like \`\`\`json.
`
        },
        {
          role: "user",
    ...
    // Clean up if the model accidentally wraps JSON with markdown
